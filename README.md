...
**Tools.** Claude wrote the implementation, tests and docs; ChatGPT/Codex reviewed.
A later Claude session built a second implementation from the brief alone, and a
reviewer agent given only the brief, both trees labelled A/B and a rubric chose
between them. A final session captured the live recording and rewrote every claim
it falsified; it added no application code.
...
**Decisions to defend.** The five reconciliation clauses; refunds as negative
spend; recompute-never-read-back; templates over prose; `Other` vs
`Uncategorized`; degraded exits 0.
