...
The first AI proposals for this brief were single-pass: give the model all four
files and have it categorise, reconcile and total them in one prompt, using as
little compute as possible. The author rejected that design before any code was
written, on two grounds. A general language model is not a validated calculator:
it will produce a plausible number, and nothing in the output distinguishes a
right one from a wrong one. And a model should not be the thing making decisions
that must not be wrong — whether a row is income or a transfer, whether two rows
are one purchase — because those decisions move the totals and a wrong answer is
invisible.

The direction given instead was the one this tree implements: the important work
— parsing, kind, identity, arithmetic — is code; the model does tool selection and
the one semantic task whose errors cannot change a total; and every model output
passes a safeguard (whitelist, contract check, template validation) before it can
affect the report. The cost is compute: a dozen model calls instead of one or two.
The benefit is a report in which no number was generated by a model, and in which
model mistakes become observations and warnings rather than silent errors.

The live run is the clearest evidence for the decision. At step 10 the model wrote
"0 items are flagged for review" when nine were. Under the rejected design that
sentence would have been the report's summary. Under this one, `validate_summary`
rejected it and code wrote the sentence from computed totals.
